...
<p align="center">
  <img src="assets/demo.gif" width="900" alt="Запись терминала: возобновлённая сессия Claude Code выполняет /tattoo, затем /compact, затем отвечает на вопрос, где хранить ключи идемпотентности и что отвергли: Postgres, Redis отвергнут; и на вопрос, можно ли пушить в main: нет, только черновые PR.">
</p>

Настоящая сессия с подключённым плагином, записана через [vhs](https://github.com/charmbracelet/vhs)
(`assets/demo.tape`). Ожидание записи татуировки и сжатия вырезано.
...
**Claude Code и так перечитывает некоторые файлы после сжатия. Зачем хук?**
Сразу после `/tattoo` и `/compact` видно, как Claude Code сам подтягивает
татуировку (`Read …/tattoo/<session>.md`, как в демо): это один из последних
файлов, которых касалась сессия. Это удобство, а не гарантия: механизм берёт
недавно использованные файлы, так что автосжатие через час после татуировки,
после десятков других файлов, может её не захватить. Хук срабатывает после
каждого сжатия и возвращает весь файл, при необходимости частями.
...
assets/demo-setup.sh && assets/demo-setup.sh --render && assets/demo-setup.sh --cleanup   # перезаписать assets/demo.gif (vhs, настоящий Claude Code)
```
